<script>
	...
	export let data;
	let Lecturas = data.res;
	let respLecturas = data.res;
	...
	function changeLevel(level) {
		markLevel = level;
		Lecturas = Filtrar(respLecturas, level);
	}
	const Filtrar = (lecturas, level) => {
		return lecturas.filter((lectura) => lectura.nivel == level);
	};
</script>

<div class=" mt-5 grid justify-items-center">
	<div class=" bg-slate-400/20 w-full md:w-1/2 rounded-md">
		<h3 class=" text-2xl mt-5 mb-5 font-mono text-center">Conversation</h3>
		...
		<div class="overflow-x-auto max-h-[600px]">
			<List>
				{#each Lecturas as lectura}
					<ListItem title={lectura.title} level={lectura.nivel} tema={lectura.tema} />
				{/each}
			</List>
		</div>
	</div>
